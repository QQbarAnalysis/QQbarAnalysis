...
TH1F *polarized_histo(TH1F *left, TH1F* right, TString pol="left") {
 ...
 if(pol=="left") {
   histo->Add(left,0.58/(13.15*0.05));
   histo->Add(right,0.035/(19.9*0.05));
 } else {
   histo->Add(left,0.035/(13.15*0.05));
   histo->Add(right,0.58/(19.9*0.05));
 }
  return histo;
}

void AfbCalc_fixedK_radret() {
  ...
  for(int ipol=0; ipol<2; ipol++){
    ...
    TString filename = TString::Format("%sbbbar_Kgamma_gt_35_radret_genkt_R125_cuts11_250GeV_%s_btag1_0.8_btag2_0.8_nbins40.root",folder.Data(),pol.Data());
//bbbar_Kgamma35_genkt_restorer_cuts11_250GeV_%s_btag1_0.8_btag2_0.8_nbins40.root",folder.Data(),pol.Data());
  ...
  g_Afbb[0]->Scale(1./(13.15*0.05));
  g_Afbb[1]->Scale(1./(19.9*0.05));
