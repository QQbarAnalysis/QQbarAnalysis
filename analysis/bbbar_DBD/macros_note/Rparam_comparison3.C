...
void Rparam_comparison3() {
  ...
  leg1->SetHeader("Jet R-parameter=1.25");
   ...
   for(int j=0; j<3; j++) {
    ...
    if(j==0) leg1->AddEntry(h_costheta_all[j][1],"Before jet cleaning","l");
    if(j==1) leg1->AddEntry(h_costheta_all[j][1],"After jet cleaning","l");
    if(j==2) leg1->AddEntry(h_costheta_all[j][1],"1.0 < K_{#gamma}^{reco.,1}/K_{#gamma}^{reco.,geom} + K_{#gamma}^{reco.,1}/K_{#gamma}^{reco.,geom} < 2.5","l");
    if(j==3) leg1->AddEntry(h_costheta_all[j][1],"Both jets with |cos(#theta)|<0.97","l");
  ...
  for(int j=0; j<3; j++) {
    ...
    if(j==0) leg2->AddEntry(h_costheta_radreturn[j][1],"Before jet cleaning","l");
    if(j==1) leg2->AddEntry(h_costheta_radreturn[j][1],"After jet cleaning","l");
    if(j==2) leg2->AddEntry(h_costheta_radreturn[j][1],"1.0 < K_{#gamma}^{reco.,1}/K_{#gamma}^{reco.,geom} + K_{#gamma}^{reco.,1}/K_{#gamma}^{reco.,geom} < 2.5","l");
    if(j==3) leg2->AddEntry(h_costheta_radreturn[j][1],"Both jets with |cos(#theta)|<0.97","l");
