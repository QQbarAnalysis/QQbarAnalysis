...
void Labels(int i=0, TString pol="eL"){
  ...
  if(i==0) QQBARLabel2(0.05,0.07, "[No Cuts]",kOrange+3);
  else QQBARLabel2(0.05,0.07, TString::Format("Cuts: %i",i),kOrange+3);
  /* if(i==1) QQBARLabel2(0.05,0.07, "K_{acol}<35 GeV",kOrange+3);
  if(i==2) QQBARLabel2(0.05,0.07, "K_{acol}<35 GeV & m_{j_{1},j_{2}}>135 GeV",kOrange+3);
  if(i==3) QQBARLabel2(0.05,0.07, "K_{acol}<35 GeV & m_{j_{1},j_{2}}>135 GeV & E^{max}_{nPFO}<100 GeV",kOrange+3);
  if(i==4) {
    QQBARLabel2(0.05,0.07, "K_{acol}<35 GeV & m_{j_{1},j_{2}}>135 GeV & E^{max}_{nPFO}<100 GeV",kOrange+3);
    QQBARLabel2(0.05,0.03, "& |cos(#theta_{E^{max}_{nPFO}})|<0.95",kOrange+3);
  }
  if(i==5) {
    QQBARLabel2(0.05,0.07, "K_{acol}<35 GeV & m_{j_{1},j_{2}}>135 GeV & E^{max}_{nPFO}<100 GeV",kOrange+3);
    QQBARLabel2(0.05,0.03, "& |cos(#theta_{E^{max}_{nPFO}})|<0.95 & m_{j_{1}}+m_{j_{2}}<90 GeV",kOrange+3);
  }
  if(i==6) {
    QQBARLabel2(0.05,0.07, "K_{acol}<35 GeV & m_{j_{1},j_{2}}>135 GeV & E^{max}_{nPFO}<100 GeV",kOrange+3);
    QQBARLabel2(0.05,0.03, "& |cos(#theta_{E^{max}_{nPFO}})|<0.95 & Thrust > 0.9",kOrange+3);
    }*/

  if(pol=="eL")QQBARLabel2(0.2,0.97, "e_{L}^{-}e_{R}^{+} #rightarrow q#bar{q}, q=udscb, 250 GeV, 250fb^{-1}",kGray+2);
  if(pol=="eR")QQBARLabel2(0.2,0.97, "e_{R}^{-}e_{L}^{+} #rightarrow q#bar{q}, q=udscb, 250 GeV, 250fb^{-1}",kGray+2);
}



void selection_plots_qq() {
  ...
  TString folder=TString::Format("../output_Cuts/selection_cuts%i_",cuts);
  TString kt="genkt_restorer";//"_genkt";//"_tight_";

  TString filename = folder+"bbbar_radret_genkt_R125_Kgamma50_250GeV_"+pol+".root";//folder+"bbbar_"+kt+sel+"250GeV_"+pol+".root";
  TFile *f = new TFile(filename);
  TH1F *h_mjj_bb = (TH1F*)f->Get("h_mjj_bb");
  TH1F *h_mj1_mj2_bb = (TH1F*)f->Get("h_mj1_mj2_bb");
  TH1F *h_y23_bb = (TH1F*)f->Get("h_y23_bb");
  TH1F *h_egamma_bb = (TH1F*)f->Get("h_egamma2_bb");
  TH1F *h_K_bb = (TH1F*)f->Get("h_K_bb");
  TH1F *h_thrust_bb = (TH1F*)f->Get("h_thrust_bb");
  TH1F *h_costhetagamma_bb = (TH1F*)f->Get("h_costhetagamma2_bb");

  TH1F *h_mjj_cc = (TH1F*)f->Get("h_mjj_cc");
  TH1F *h_mj1_mj2_cc = (TH1F*)f->Get("h_mj1_mj2_cc");
  TH1F *h_y23_cc = (TH1F*)f->Get("h_y23_cc");
  TH1F *h_egamma_cc = (TH1F*)f->Get("h_egamma2_cc");
  TH1F *h_K_cc = (TH1F*)f->Get("h_K_cc");
  TH1F *h_thrust_cc = (TH1F*)f->Get("h_thrust_cc");
  TH1F *h_costhetagamma_cc = (TH1F*)f->Get("h_costhetagamma2_cc");

  TH1F *h_mjj_qq = (TH1F*)f->Get("h_mjj_qq");
  TH1F *h_mj1_mj2_qq = (TH1F*)f->Get("h_mj1_mj2_qq");
  TH1F *h_y23_qq = (TH1F*)f->Get("h_y23_qq");
  TH1F *h_egamma_qq = (TH1F*)f->Get("h_egamma2_qq");
  TH1F *h_K_qq = (TH1F*)f->Get("h_K_qq");
  TH1F *h_thrust_qq = (TH1F*)f->Get("h_thrust_qq");
  TH1F *h_costhetagamma_qq = (TH1F*)f->Get("h_costhetagamma2_qq");

  cout<<filename<<endl;
  /* if(pol=="eL" ) { 
    h_mjj_bb->Scale(250./12.9);
    h_mj1_mj2_bb->Scale(250./12.9);
    h_y23_bb->Scale(250./12.9);
    h_egamma_bb->Scale(250./12.9);
    h_K_bb->Scale(250./12.9);
    h_thrust_bb->Scale(250./12.9);
    h_costhetagamma_bb->Scale(250./12.9);

    h_mjj_cc->Scale(250./12.9);
    h_mj1_mj2_cc->Scale(250./12.9);
    h_y23_cc->Scale(250./12.9);
    h_egamma_cc->Scale(250./12.9);
    h_K_cc->Scale(250./12.9);
    h_thrust_cc->Scale(250./12.9);
    h_costhetagamma_cc->Scale(250./12.9);

    h_mjj_qq->Scale(250./12.9);
    h_mj1_mj2_qq->Scale(250./12.9);
    h_y23_qq->Scale(250./12.9);
    h_egamma_qq->Scale(250./12.9);
    h_K_qq->Scale(250./12.9);
    h_thrust_qq->Scale(250./12.9);
    h_costhetagamma_qq->Scale(250./12.9);    
  }

  if(pol=="eR" ) {
    h_mjj_bb->Scale(250./17.95);
    h_mj1_mj2_bb->Scale(250./17.95);
    h_y23_bb->Scale(250./17.95);
    h_egamma_bb->Scale(250./17.95);
    h_K_bb->Scale(250./17.95);
    h_thrust_bb->Scale(250./17.95);
    h_costhetagamma_bb->Scale(250./17.95);

    h_mjj_cc->Scale(250./17.95);
    h_mj1_mj2_cc->Scale(250./17.95);
    h_y23_cc->Scale(250./17.95);
    h_egamma_cc->Scale(250./17.95);
    h_K_cc->Scale(250./17.95);
    h_thrust_cc->Scale(250./17.95);
    h_costhetagamma_cc->Scale(250./17.95);

    h_mjj_qq->Scale(250./17.95);
    h_mj1_mj2_qq->Scale(250./17.95);
    h_y23_qq->Scale(250./17.95);
    h_egamma_qq->Scale(250./17.95);
    h_K_qq->Scale(250./17.95);
    h_thrust_qq->Scale(250./17.95);
    h_costhetagamma_qq->Scale(250./17.95);
    }*/


  // ----------------------------------------
  //    TString filename = folder+"bbbar_radret_genkt_R050_Kgamma35_250GeV_"+pol+".root";//folder+"bbbar_"+kt+sel+"250GeV_"+pol+".root";

  //  filename = folder+"bbbar_radret_Kgamma35_250GeV_"+pol+".root";
  ...
  TH1F *h_egamma_radreturn = (TH1F*)f1->Get("h_egamma2_radreturn");
  TH1F *h_K_radreturn = (TH1F*)f1->Get("h_K_radreturn");
  TH1F *h_thrust_radreturn = (TH1F*)f1->Get("h_thrust_radreturn");
  TH1F *h_costhetagamma_radreturn = (TH1F*)f1->Get("h_costhetagamma2_radreturn");

  /*  if(pol=="eL" ) { 
    h_mjj_radreturn->Scale(250./12.9);
    h_mj1_mj2_radreturn->Scale(250./12.9);
    h_y23_radreturn->Scale(250./12.9);
    h_egamma_radreturn->Scale(250./12.9);
    h_K_radreturn->Scale(250./12.9);
    h_thrust_radreturn->Scale(250./12.9);
    h_costhetagamma_radreturn->Scale(250./12.9);
  }

  if(pol=="eR" ) {
    h_mjj_radreturn->Scale(250./17.95);
    h_mj1_mj2_radreturn->Scale(250./17.95);
    h_y23_radreturn->Scale(250./17.95);
    h_egamma_radreturn->Scale(250./17.95);
    h_K_radreturn->Scale(250./17.95);
    h_thrust_radreturn->Scale(250./17.95);
    h_costhetagamma_radreturn->Scale(250./17.95);
    }*/
  ...
  cout<<std::setprecision(3)<< cut_string<<" & "<< 100.*bb_integral[i+1]/bb_integral[0]<<" & "<<100.*cc_integral[i+1]/cc_integral[0]<<" & "<<100.*qq_integral[i+1]/qq_integral[0]<<" & "<<100.*radreturn_integral[i+1]/radreturn_integral[0]<<" \\\\"<<endl;
  cout<<std::setprecision(3)<< cut_string<<" & "<< bb_integral[i+1]<<" & "<<cc_integral[i+1]<<" & "<<qq_integral[i+1]<<" & "<<radreturn_integral[i+1]<<" \\\\"<<endl;
  
  cout<<std::setprecision(3)<< cut_string<<" & "<< 100.*bb_integral[i+1]/bb_integral[i+1]<<" & "<<100.*cc_integral[i+1]/bb_integral[i+1]<<" & "<<100.*qq_integral[i+1]/bb_integral[i+1]<<" & "<<100.*radreturn_integral[i+1]/bb_integral[i+1]<<" \\\\"<<endl;
  ...
  TCanvas * canvasy23 = new TCanvas("canvas_y23","canvas_y23",800,800);
  canvasy23->cd(1);
  gPad->SetLogy();
  ...
  TLegend *leg1 = new TLegend(0.3,0.85,0.5,0.93);//(0.4,0.3,0.5,0.6);
  ...
  //canvasy23->Print(TString::Format("plots/nobkg_y23_%s_%s_cuts%i.eps",pol.Data(),kt.Data(),cuts));
  ...
  //canvasK->Print(TString::Format("plots/nobkg_K_%s_%s_cuts%i.eps",pol.Data(),kt.Data(),cuts));
  ...
  h_thrust_bb->GetXaxis()->SetRangeUser(0.5,1.0);
  ...
  h_egamma_bb->GetXaxis()->SetTitle("E^{max}_{nPFO} [GeV]");
  h_egamma_bb->SetLineColor(4);
  h_egamma_bb->GetYaxis()->SetRangeUser(0,h_egamma_bb->GetMaximum()*2);
