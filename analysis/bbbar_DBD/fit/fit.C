...
TF1 * fit_histo_bkg(TH1F * histo, TH1F * bkg, float lumi_factor) {
...
void fit() {
  ...
  //for(int i=0; i<20; i++) h_eff->SetBinContent(i+1,h_eff->GetBinContent(40-i));
  h_eff->Scale(1./max);
  ...
  for(int i=nbins/2-1; i<nbins; i++) {
    ...
    // if(unfres->GetBinError(i+1)>0) {
    //   double value_a = (fitresult2->Eval(unfres->GetBinCenter(i+1))-unfres->GetBinContent(i+1)) /unfres->GetBinError(i+1);
    //   double w_a= unfres->GetBinContent(i+1);
    //   double value_b = (fitresult2->Eval(unfres->GetBinCenter(-nbins/2+i+1))-unfres->GetBinContent(-nbins/2+i+1))/unfres->GetBinError(-nbins/2+i+1);
    //   double w_b= unfres->GetBinContent(-nbins/2+i+1);
    //   pull4->SetBinContent(i+1,(value_a*w_a+value_b+w_b)/(w_a+w_b));
    // } else {
    //   pull4->SetBinContent(i+1,1);
    //   }
   }
     
  // for(int i=0; i<40; i++) {
  //   if(h_corrected->GetBinError(i+1)>0) {
  //     pull->SetBinContent(i+1,(fitresult3->Eval(h_corrected->GetBinCenter(i+1)) - h_corrected->GetBinContent(i+1)) /h_corrected->GetBinError(i+1));
  //   } else {
  //     pull->SetBinContent(i+1,1);
  //   }
  //   if(h_noncorrected->GetBinError(i+1)>0) {
  //      pull2->SetBinContent(i+1,(fitresult3->Eval(h_corrected->GetBinCenter(i+1)) - h_noncorrected->GetBinContent(i+1)) /h_noncorrected->GetBinError(i+1) );
  //   } else {
  //     pull2->SetBinContent(i+1,1);
  //   }

  //   if(h_parton->GetBinError(i+1)>0) {
  //     pull3->SetBinContent(i+1,(fitresult1->Eval(h_parton->GetBinCenter(i+1)) - h_parton->GetBinContent(i+1)) /h_parton->GetBinError(i+1) );
  //   } else {
  //     pull3->SetBinContent(i+1,1);
  //   }
  // }
  ...
  if(pol=="eL") pull->SetTitle("e_{L}^{+}e_{R}^{-}#rightarrow b#bar{b} @ 250GeV, 250 fb^{-1}");
  ...
  h_eff->GetXaxis()->SetTitle("|cos(#theta)|");
  h_eff->GetXaxis()->SetRangeUser(0,1);
  h_eff->GetYaxis()->SetTitle("(correction factor)^{-1}");
  h_eff->SetTitle("");
  h_eff->GetYaxis()->SetTitleOffset(1.6);
  h_eff->GetYaxis()->SetRangeUser(0.8,1.05);
  
  h_eff->Draw("p");

  TLegend *leg3 = new TLegend(0.5,0.25,0.93,0.4);
  leg3->AddEntry(h_eff,"Det. acceptance correction ","l");
  leg3->SetFillColor(0);
  leg3->SetLineColor(0);
