...
void Labels(TString pol="eL"){
  ...
  if(pol=="eL")QQBARLabel2(0.17,0.97, "e_{L}^{-}e_{R}^{+} #rightarrow q#bar{q} (q=udscb, E_{#gamma}<35 GeV)",kGray+2);
  if(pol=="eR")QQBARLabel2(0.17,0.97, "e_{R}^{-}e_{L}^{+} #rightarrow q#bar{q} (q=udscb,E_{#gamma}<35 GeV)",kGray+2);
  if(pol=="both")QQBARLabel2(0.17,0.97, "e^{-}e^{+} #rightarrow q#bar{q} (q=udscb, E_{#gamma}<35 GeV)",kGray+2);
...
void eff_rho_plots2() {
  ...
  TGraphErrors* rho_btagR= (TGraphErrors*)f->Get("rhoLEP");
  TGraphErrors* rho_BcR= (TGraphErrors*)f->Get("rhoLEP_Bc");
  TGraphErrors* rho_KcR= (TGraphErrors*)f->Get("rhoLEP_Kc");

  TGraphErrors* effb_btagR= (TGraphErrors*)f->Get("effb");
  TGraphErrors* effb_BcR= (TGraphErrors*)f->Get("effb_Bc");
  TGraphErrors* effb_KcR= (TGraphErrors*)f->Get("effb_Kc");
  TGraphErrors* eff2jR= (TGraphErrors*)f->Get("eff2j");

  
  pol="eL";
  filename = "../output/correlations_quark5_250GeV_"+pol+"_btag1_0.8_btag2_0.8_nbins40.root";
  cout<<filename<<endl;
  ...
  canvas_eff->Print("eff_btag_charge.eps");
  ...
  canvas_rho->Print("rho_btag_charge.eps");
