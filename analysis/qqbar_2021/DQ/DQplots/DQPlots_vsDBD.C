...
void DQPlots_vsDBD() {
  ...
  for(int i=0; i<nfiles; i++) {
    for(int j=0; j<6; j++) {
      ...
      if(newfile[i]==false) histos_Nreco.push_back(getHisto("reco",file[i],"PFO_njet"+names[j]));
      else  histos_Nreco.push_back(getHisto("reco",file[i],"PFO_njet"+names[j],"PFOoverlay_njet"+names[j],"PFOisr_njet"+names[j]));
    }
    ...
    if(newfile[i]==false)  histos_Nreco_all.push_back(getHisto("reco",file[i],"PFO_njet"+names[6]));
    else  histos_Nreco_all.push_back(getHisto("reco",file[i],"PFO_njet"+names[6],"PFOoverlay_njet"+names[6],"PFOisr_njet"+names[6]));
  } 
