...
void plot2models() {
 ...
 // large model
  ...
  // small model
  //---------------------------------
  model="s5_valencia2.0";
  filename = TString::Format("%s/bbbar_%s_cuts3_500GeV_%s_btag1_0.9_btag2_0.2_nbins%i.root",folder.Data(),model.Data(),pol.Data(),nbins);
  ...
  for(int i=0; i<nbins+2; i++) {  
     ...
     double error = (h2_corrected->GetBinContent(i)/h2_eff->GetBinContent(i))*sqrt(pow(h2_corrected->GetBinError(i)/h2_corrected->GetBinContent(i),2)+0.25*pow(h2_eff->GetBinError(i) /h2_eff->GetBinContent(i),2));
    error= sqrt(error*error +  0.0*0.01*pow(h2_corrected->GetBinContent(i)/h2_eff->GetBinContent(i)-h2_corrected->GetBinContent(i),2));
    //double error = sqrt( pow((h_corrected->GetBinContent(i)/h_eff->GetBinContent(i))*
